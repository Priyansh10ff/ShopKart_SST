...
const ProductDetails = () => {
  ...
  const handleWishlist = async () => {
    try {
      const response = await axiosInstance.post(`/customers/wishlist/${id}`);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      {loading && (
        <h2 className="text-center text-lg text-[#777]">Loading product...</h2>
      )}
      ...
      {!loading && !error && !product && (
        <h2 className="text-center text-lg text-[#777]">Product not found.</h2>
      )}
      ...
      {!loading && !error && product && (
        <div className="grid grid-cols-1 gap-8 overflow-hidden rounded-3xl bg-white shadow-sm md:grid-cols-2">
          ...
          <div className="flex flex-col justify-center p-6 sm:p-8 lg:p-10">
            ...
            <p className="mt-6 leading-7 text-[#666]">{product.description}</p>
            ...
            <button
              type="button"
              className="mt-3 w-full rounded-full border border-[#303030] px-6 py-3 font-medium text-[#303030] transition hover:bg-[#303030] hover:text-white"
            >
              Add to Cart
            </button>
          </div>
